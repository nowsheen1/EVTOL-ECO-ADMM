...
function design_coupling_eco_admm()
...
rho3=0.1;
    ...
    [f, c, ~] = computePerformance_ecoadmm(x0, vehicle, range, payload);

    opt = optimoptions('fmincon', 'Display', 'final-detailed', 'MaxIterations', 500, 'ScaleProblem', false, 'MaxFunctionEvaluations', 10000);
    [xopt, fopt, eopt, oopt] = fmincon(@(x) objfun(x, vehicle, range, payload), x0, [], [], [], [], lb, ub, @(x) constr(x, vehicle, range, payload), opt);
    
    format long
    [fopt, copt, ~] = computePerformance_ecoadmm(xopt, vehicle, range, payload)
    ...
    for idx = 1:length(x0)
        ...
        [fpdx, cpdx, ~] = computePerformance_ecoadmm(xpdx, vehicle, range, payload);
        [fmdx, cmdx, ~] = computePerformance_ecoadmm(xmdx, vehicle, range, payload);
...
%      ax.YAxis.TickLabelColor = [0 0 0];
    ...
    function y = objfun(x, vehicle, range, payload)
        if ~isequal(x,xLast) % Check if computation is necessary
            [myf,myc,myceq] = computePerformance_ecoadmm(x, vehicle, range, payload);
            xLast = x;
        end
